...
func TestRenderSessionRefs(t *testing.T) {
	a := &App{}
	// No refs at all.
	if got := a.renderSessionRefs(nil, 80, false); !strings.Contains(got, "No PR or Jira") {
		t.Errorf("empty render missing hint: %q", got)
	}
	// Has links but not yet resolved.
	if got := a.renderSessionRefs(nil, 80, true); !strings.Contains(got, "Resolving") {
	...
	out := a.renderSessionRefs(orderRefs(refs), 80, true)
	...
	a.sessRefsCursor = 0
	if got := a.renderSessionRefs(orderRefs(refs), 80, true); !strings.Contains(got, "> ") {
		t.Errorf("focused render missing cursor marker:\n%s", got)
	}
	if !strings.Contains(a.renderSessionRefs(orderRefs(refs), 80, true), "↵:open") {
