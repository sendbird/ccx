...
// generateTestCLAUDEmd creates a CLAUDE.md in the test env that embeds
// selected memory files inline. This avoids broken @~/.claude/... references
// since the test env uses a fake HOME.
func generateTestCLAUDEmd(configDir string, items []session.ConfigItem) {
	var b strings.Builder
	b.WriteString("# Test Environment\n\n")
	b.WriteString("Generated by ccx for config testing.\n\n")
	...
	for _, item := range items {
		...
		// Use the item name as a section identifier
		name := item.Name
		if name == "CLAUDE.md" {
			// Embed the original CLAUDE.md content directly
