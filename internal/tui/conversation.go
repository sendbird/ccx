...
func (a *App) renderConvImagesPage(width int) string {
	var lines []string
	for _, e := range a.conv.messages {
		for _, b := range e.Content {
			if b.Type == "image" && b.ImagePasteID > 0 {
				...
				lines = append(lines, fmt.Sprintf("[%s] paste #%d", label, b.ImagePasteID))
			}
		}
	}
	return renderConvArtifactPage("Images", lines, width)
}

func (a *App) renderConvURLsPage(width int) string {
	var lines []string
	seen := make(map[string]bool)
	...
	return renderConvArtifactPage("URLs", lines, width)
}

func (a *App) renderConvFilesPage(width int) string {
	var lines []string
	seen := make(map[string]bool)
	for _, m := range a.conv.merged {
		for _, item := range extract.BlockFilePaths(m.entry.Content) {
	...
	return renderConvArtifactPage("Files", lines, width)
}

func (a *App) renderConvChangesPage(width int) string {
	var lines []string
	seen := make(map[string]bool)
	for _, m := range a.conv.merged {
		for _, item := range extract.BlockChanges(m.entry.Content) {
			if !seen[item.Item.URL] {
				seen[item.Item.URL] = true
				lines = append(lines, item.Item.URL)
			}
		}
	}
	return renderConvArtifactPage("Changes", lines, width)
}

func renderConvArtifactPage(title string, items []string, width int) string {
	if len(items) == 0 {
		return dimStyle.Render("No " + strings.ToLower(title))
	}
	var sb strings.Builder
	sb.WriteString(dimStyle.Render("── "+title+" ──") + "\n\n")
	for _, item := range items {
		sb.WriteString("  " + wrapText(item, max(width-4, 10)) + "\n\n")
	}
	return sb.String()
}
