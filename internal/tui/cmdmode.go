...
func buildCmdRegistry() []cmdEntry {
	return []cmdEntry{
		// Group modes
		{
			name: "group:flat", aliases: []string{"g:flat"},
			desc: "flat view",
			action: func(a *App) (tea.Model, tea.Cmd) {
		...
		{
			name: "group:proj", aliases: []string{"g:proj"},
			desc: "project groups",
			action: func(a *App) (tea.Model, tea.Cmd) {
		...
		{
			name: "group:tree", aliases: []string{"g:tree"},
			desc: "tree view",
			action: func(a *App) (tea.Model, tea.Cmd) {
				a.sessGroupMode = groupTree
				a.rebuildSessionList()
				return a, nil
			},
		},
		{
			name: "group:chain", aliases: []string{"g:chain"},
			desc: "chain groups",
			action: func(a *App) (tea.Model, tea.Cmd) {
				a.sessGroupMode = groupChain
				a.rebuildSessionList()
				return a, nil
			},
		},
		{
			name: "group:fork", aliases: []string{"g:fork"},
			desc: "fork groups",
			action: func(a *App) (tea.Model, tea.Cmd) {
				a.sessGroupMode = groupFork
				a.rebuildSessionList()
				return a, nil
			},
		},
		...
		// Preview modes
		{
			name: "preview:conv", aliases: []string{"p:conv"},
			desc: "conversation preview",
			action: func(a *App) (tea.Model, tea.Cmd) {
		...
		{
			name: "preview:stats", aliases: []string{"p:stats"},
			desc: "stats preview",
			action: func(a *App) (tea.Model, tea.Cmd) {
		...
		{
			name: "preview:mem", aliases: []string{"p:mem"},
			desc: "memory preview",
			action: func(a *App) (tea.Model, tea.Cmd) {
		...
		{
			name: "preview:tasks", aliases: []string{"p:tasks"},
			desc: "tasks preview",
			action: func(a *App) (tea.Model, tea.Cmd) {
		...
		{
			name: "preview:live", aliases: []string{"p:live"},
			desc: "live preview",
			action: func(a *App) (tea.Model, tea.Cmd) {
				a.setSessPreviewMode(sessPreviewLive)
				return a, nil
			},
		},
		...
		// Actions
		{
			name: "refresh", aliases: []string{"R"},
			desc: "refresh sessions",
			action: func(a *App) (tea.Model, tea.Cmd) {
				cmd := a.doRefresh()
		...
		// Config
		{
			name: "keymap:edit", aliases: []string{"km:edit"},
			desc: "edit keymap config",
			action: func(a *App) (tea.Model, tea.Cmd) {
				return a.bootstrapAndEditKeymap()
			},
		},
	}
}
...
// updateCmdSuggestions filters the command registry based on current input.
// When input is empty, shows a curated set of top-level categories.
